...
bool checkbianjian(int i,int j,int w,int h)
{
  if (i>=0&&i<h&&j>=0&&j<w)
    return true;
  return false;
...
//Only for Entire Image
void bulge(IplImage* frame, int startX,int startY,int endX,int endY)
{
  ...
  for (int i = 0; i < frame->width; i++)
  {
    for (int j = 0; j < frame->height; j++)
    {
      int tempi= i; 
      int tempj = j;
      new_data[frame->widthStep * j + i * 3] =  old_data[frame->widthStep * tempj + tempi * 3];
      new_data[frame->widthStep * j + i * 3 + 1] =  old_data[frame->widthStep * tempj + tempi * 3 + 1];
      new_data[frame->widthStep * j + i * 3 + 2] =  old_data[frame->widthStep * tempj + tempi * 3 + 2];
    }
  }
  ...
  for (int i = startX; i < endX; i++)
  {
    for (int j = startY; j < endY; j++)
    {
      ...
      double para_r = sqrt( pow(x,2) + pow(y,2));
      double para_a = atan2( y,x);
      double para_rn =0.11* pow(para_r,2.4)/width;
      newX= (int)(para_rn*cos(para_a) + center_X); 
      newY =(int)(para_rn*sin(para_a) + center_Y);
      
      if (newX< frame->width && newY< frame->height && newX >0 && newY>0)
      {
...
//For Image and Video, sqeeze Effect(Entire or partial)
void jiya(IplImage *image,int chengdu,int left,int top,int right, int bottom)//挤压
{ 
  if(right-left-chengdu<1)
    return;
  IplImage* initimage = cvCloneImage( image );
  
  CvScalar s;
  double bili1;
  int size;
  //左右挤压，上半部分
  for(int i = top+1;i < (top+bottom)/2;i++)
  {
    bili1=((double)(bottom-top)/2)/(i-top);
    size=(int)((right-left-chengdu)/2/bili1);
    double bili=((double)(right-left+1))/(right-left+1-size*2);//缩小的倍数
    for(int j = left+size;j < right-size;j++)
    {
      if(i>=image->height||j>=image->width)
	break;
      
      if(checkbianjian(i, (int)((j-size-left)*bili+left+0.5),image->width, image->height))
	s=cvGet2D(initimage,i,(int)((j-size-left)*bili+left+0.5));//反向查询现在该点对应原图哪个点
	
	if(checkbianjian(i, j,image->width, image->height))
	  cvSet2D(image,i,j,s);
    }
    if(checkbianjian(i, left-1,image->width, image->height))
      s=cvGet2D(initimage,i,left-1);  
    for(int j = left;j <left+size;j++)
      if(checkbianjian(i, j,image->width, image->height))
	cvSet2D(image,i,j,s);
      
      if(checkbianjian(i,right,image->width, image->height))
	s=cvGet2D(initimage,i,right);
      for(int j = right-size;j <right;j++)
	if(checkbianjian(i,j,image->width, image->height))
	  cvSet2D(image,i,j,s);
  }
  //From left/right, bottom part
  for(int i = (top+bottom)/2;i < bottom;i++)
  {
    if(i==(top+bottom)/2)
      bili1=1;
    else
      bili1=((double)(bottom-top)/2)/(bottom-i);
    size=(int)((right-left-chengdu)/2/bili1);
    double bili=((double)(right-left+1))/(right-left+1-size*2);//缩小的倍数
    for(int j = left+size;j < right-size;j++)
    {
  ...
  cvReleaseImage(&initimage);
}

//Video and image(Entire or partial)
void xuanwo(IplImage *image,int left ,int top,int right,int bottom)//漩涡
{
  static int maxr;
  IplImage* initimage = cvCloneImage( image );
  CvScalar s;
  int MaxR,i,j,tempx,tempy;
  double offset,radius;
  int CenterY=(left+right)/2;
  int CenterX=(top+bottom)/2;
  if((right-left)>(bottom-top))
    return;
  int cha=(right-left)-(bottom-top);
  MaxR=(bottom-top)/2;
  bool isok=true;
  if(CenterY-MaxR<=1)
    isok=false;
  if (CenterY+MaxR>=image->width-2)
    isok=false;
  if(CenterX-MaxR<=1)
    isok=false;
  if (CenterX+MaxR>=image->height-2)
    isok=false;
  if (isok)
    maxr=MaxR;
  double follow=200.0/(maxr*sqrt((double)maxr));
  
  
  for(i=CenterX-maxr;i<=CenterX+maxr;i++)
  {
    if(i==CenterX)
      continue;
    for(j=CenterY-maxr;j<=CenterY+maxr;j++)
    {
      
      radius=sqrt((double)((i-CenterX)*(i-CenterX)+(j-CenterY)*(j-CenterY)));
      if(radius<=maxr)
      { 
	offset=200.0/(radius*sqrt((double)radius))-follow;
	tempx=(int)(CenterX-(j-CenterY)*sin(offset)+(i-CenterX)*cos(offset));
	tempy=(int)(CenterY+(j-CenterY)*cos(offset)+(i-CenterX)*sin(offset));
	if(checkbianjian(tempx, tempy,image->width, image->height))
	  s=cvGet2D(initimage,tempx,tempy);
	if(checkbianjian(i-CenterX+maxr+top, j-CenterY+maxr+left+(cha/2),image->width, image->height))
	  cvSet2D(image,i-CenterX+maxr+top,j-CenterY+maxr+left+(cha/2),s);
      }
      else
      {
	if(checkbianjian(i, j,image->width, image->height))
	  s=cvGet2D(initimage,i,j);
	if(checkbianjian(i-CenterX+maxr+top, j-CenterY+maxr+left+(cha/2),image->width, image->height))
	  cvSet2D(image,i-CenterX+maxr+top,j-CenterY+maxr+left+(cha/2),s);
      }
    }
...
//For video and image(Entire or partial)
void sumiao(IplImage *image,int left,int top,int right, int bottom)//素描
{
  CvScalar s,s1,s2,s3;
  for(int i = top;i < bottom-1;i++)
  {
    ...
    {
      s1 = cvGet2D(image,i,j); // get the (i,j) pixel value
      s2=cvGet2D(image,i+1,j); // get the (i,j) pixel value
      s3 = cvGet2D(image,i,j+1); // get the (i,j) pixel value
      double r1=(s1.val[0]-s2.val[0])*(s1.val[0]-s2.val[0]);
      double r2=(s1.val[0]-s3.val[0])*(s1.val[0]-s3.val[0]);
      int R=(int)(16*sqrt( r1+r2));
      double g1=(s1.val[1]-s2.val[1])*(s1.val[1]-s2.val[1]);
      double g2=(s1.val[1]-s3.val[1])*(s1.val[1]-s3.val[1]);
      int G=(int)(16*sqrt( r1+r2));
      double b1=(s1.val[2]-s2.val[2])*(s1.val[2]-s2.val[2]);
      double b2=(s1.val[2]-s3.val[2])*(s1.val[2]-s3.val[2]);
      int B=(int)(16*sqrt( r1+r2));
      if(R>255) R=255;
		   if(G>255) R=255;
		   if(B>255) R=255;
		   int Y=(int)(0.299*(255-R)+ 0.587*(255-G) +0.114*(255-B));
      s.val[0]=Y;
      s.val[1]=Y;
      s.val[2]=Y;
		    ...
		    double scale, bool tryflip )
{
  ...
  const static Scalar colors[] =  { CV_RGB(0,0,255),
    CV_RGB(0,128,255),
		  ...
		  //draw
		  int effectOrder = 0;
		  for( vector<Rect>::const_iterator r = faces.begin(); r != faces.end(); r++, i++, effectOrder++)
		  {
		    Mat smallImgROI;
		    ...
		    if( 0.75 < aspect_ratio && aspect_ratio < 1.3 )
		    {
		      ...
		      IplImage hahaImg;
		    hahaImg = IplImage(img); 
		    if(effectOrder %3 == 0)
		      xuanwo(&hahaImg,cvRound( r->x*scale),cvRound(r->y*scale),cvRound((r->x + r->width-1)*scale),cvRound((r->y + r->height-1)*scale));
		    else if (effectOrder %3 == 1)
		      jiya(&hahaImg,cvRound(r->width*scale)*0.2,cvRound( r->x*scale),cvRound(r->y*scale),cvRound((r->x + r->width-1)*scale),cvRound((r->y + r->height-1)*scale));
		    else 
		      sumiao(&hahaImg,cvRound( r->x*scale),cvRound(r->y*scale),cvRound((r->x + r->width-1)*scale),cvRound((r->y + r->height-1)*scale));
		    
		    //fangda(img,cvRound( r->x*scale),cvRound(r->y*scale),cvRound((r->x + r->width-1)*scale),cvRound((r->y + r->height-1)*scale));
		    //fangda(img,0,0,img.rows,img.cols);    
