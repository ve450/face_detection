...
//    cout << "here2\n";
    err = clSetKernelArg(cascadesum, 5, sizeof(int), &sum_mat_size[buf_idx]);
    err = clSetKernelArg(cascadesum, 6, sizeof(cl_mem), &mat_header_buffer[buf_idx]);
    err = clSetKernelArg(cascadesum, 7, sizeof(cl_mem), &res_buf[buf_idx]);
    err = clSetKernelArg(cascadesum, 8, sizeof(int), &num_rects[buf_idx]);
    err = clSetKernelArg(cascadesum, 9, sizeof(cl_mem), &ids_buf[buf_idx]);
    ...
    const size_t global_size[] = {num_rects[buf_idx]+16-num_rects[buf_idx]%16};
    cout << "global_size: " << num_rects[buf_idx]+16-num_rects[buf_idx]%16 << endl;
...
//cout<<"x: "<<rects[i].x<<" y: "<<rects[i].y<<" factor: " << rects[i].factor<<endl;
  ...
  //    cout << "here2\n";
      ...
      printf( "Phase2\n");
      printf( "buf transfer time = %g ms\n", cl_t1/((double)cvGetTickFrequency()*1000.) );
      const size_t global_size2[] = {num_rects[buf_idx]+16-num_rects[buf_idx]%16};
      cout << "global_size2: " << num_rects[buf_idx]+16-num_rects[buf_idx]%16 << endl;
